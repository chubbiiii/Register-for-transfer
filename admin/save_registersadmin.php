...
$serverName = "localhost";
	...
	$strSQL = "SELECT * FROM member WHERE Username = '".trim($_POST['txtUsername1'])."' ";
	$objQuery = mysqli_query($objCon,$strSQL);
	$objResult = mysqli_fetch_array($objQuery,MYSQLI_ASSOC);
	if($objResult)
	{
			echo "<body onload=\"window.alert('UserID นี้ถูกใช้งานแล้ว');return history.go(-1)\">";
	}
	else
	{	
		
		$strSQL = "INSERT INTO member (Username,Password,Name,Status) VALUES ('".$_POST["txtUsername1"]."', 
		'".$_POST["txtPassword1"]."','".$_POST["txtName1"]."','".$_POST["txtStatus1"]."')";
		$objQuery = mysqli_query($objCon,$strSQL);
